...
function update_missprob2(n, k, prevn, pk1, probk,w)
    global s alpha 
 ...
 %w=1;
    ...
    if (n == 1)
            ...
            update_missprob2(s(n).interfaces(1), k, n, pk1, pk1,s(n).weight(1));
    else
            ...
            if s(n).interfaceno >= 1, 
                ...
                update_missprob2(s(n).interfaces(1), k, n, pk1, tmpprobk, s(n).weight(1));
            end
            if s(n).interfaceno >= 2, 
                %update_missprob(s(n).interfaces(2), k, n, pk1, tmpprobk, w*s(n).weight(2));
                update_missprob2(s(n).interfaces(2), k, n, pk1, tmpprobk, s(n).weight(2));
            end
            if s(n).interfaceno >= 3, 
                %update_missprob(s(n).interfaces(3), k, n, pk1, tmpprobk, w*s(n).weight(3)); 
                update_missprob2(s(n).interfaces(3), k, n, pk1, tmpprobk, s(n).weight(3)); 
            end
            if s(n).interfaceno == 4, 
                %update_missprob(s(n).interfaces(4), k, n, pk1, tmpprobk, w*s(n).weight(4)); 
                update_missprob2(s(n).interfaces(4), k, n, pk1, tmpprobk, s(n).weight(4)); 
            end
    end
end
